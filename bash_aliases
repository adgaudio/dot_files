...
alias submodule_update='git stash |grep -E "No local changes to save$" && a=0 || a=1 ; git submodule foreach "(git fetch ; git checkout master ; git pull --ff-only origin master)"  2>/dev/null ; git commit -am "update submodules" 1>/dev/null ; [ "$a" -eq "1" ] && git stash pop 1>/dev/null'
